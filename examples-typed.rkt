...
;; Examples and tests for "mk-typed.rkt", the Typed Racket tinyKanren.
;;
;; The same cases as "examples.rkt", with the annotations Typed Racket
;; needs: a relation is a function into `Goal', and every term the
;; queries build has to be a `Term'.
;;
...
(struct kase ([what : String] [got : (Listof Term)] [want : (Listof Any)])
  #:transparent)
...
(: show (-> (Listof Any) String))
(define show
  (lambda (ts)
    (string-join (map (lambda ([t : Any]) (format "~s" t)) ts) " ")))
...
(module+ main
  ...
  (define failed (length (filter (lambda ([b : Boolean]) (not b)) results)))
